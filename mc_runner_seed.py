...
def run_simulation(config_file):
    ...
    try:
        ...
        # print(result.stdout)  # Uncomment for debugging/logging
    except subprocess.CalledProcessError as e:
        print(f"Error during simulation: {e.stderr}")

def monte_carlo_test_with_seeds(base_config_path, seed_range, num_runs):
...
if __name__ == "__main__":
    ...
    for config_file in cases:
        print(f"\nRunning Monte Carlo simulation with config: {config_file}")
        monte_carlo_test_with_seeds(config_file, seed_range, num_runs)
