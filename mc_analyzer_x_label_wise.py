...
class MCAnalyzer:
    def __init__(self, config_path):
        ...
        self.output_folder = self.config.get('image_output_folder', './results/mc_plot_results/x_label_wise')
    ...
    def plot_boxplots(self, metric_data, title, ylabel, filename):
        ...
        palette = None
        if self.y_label_colors:
            unique_labels = sorted(set(df[self.y_label]))
            color_map = sns.color_palette("tab10", len(unique_labels))
            palette = {label: color_map[self.y_label_colors[i] % len(color_map)] for i, label in enumerate(unique_labels)}

        plt.figure(figsize=(10, 5))
        sns.boxplot(data=df, x=self.x_label, y='value', hue=self.y_label, palette=palette)
    ...
    def run(self):
        ...
        self.plot_boxplots(metrics['final_times'], 'Mission Completion Time', 'Time (s)', 'mission_completion_time.png')
        # self.plot_boxplots(metrics['final_distances'], 'Sum(Agents Distance Moved)', 'Distance', 'total_distance_moved.png')
        # self.plot_boxplots(metrics['final_tasks_done'], 'Sum(Agents Task Amount Done)', 'Tasks Done', 'total_task_amount_done.png')
        # self.plot_boxplots(metrics['gini_coeff_task_amount_done'], 'Gini Coefficient for Task Amount Done', 'Gini Coefficient', 'gini_task_amount_done.png')
        # self.plot_boxplots(metrics['gini_coeff_distance_moved'], 'Gini Coefficient for Distance Moved', 'Gini Coefficient', 'gini_distance_moved.png')
        # self.plot_boxplots(metrics['average_task_amount_done_per_agent'], 'Average Task Amount Done Per Agent', 'Tasks Done', 'agent_task_amount_done.png')
        ...
        # self.plot_boxplots(metrics['std_task_amount_done'], 'Std/Ave of Task Amount Done', 'Coefficient of Variation', 'std_task_amount_done.png')
        # self.plot_boxplots(metrics['std_distance_moved'], 'Std/Ave of Distance Moved', 'Coefficient of Variation', 'std_distance_moved.png')
