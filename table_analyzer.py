...
class CombinedTableAnalyzer:
    def __init__(self, yaml_path):
        ...
        self.analyzers = [
            (DistanceMovedTableAnalyzer(self.config), "Average Distance Moved Table", "average_distance_moved_table.png"),
            (MissionCompletionTableAnalyzer(self.config), "Mission Completion Time Table", "mission_completion_table.png"),
            (TransportationSuccessTableAnalyzer(self.config), "Transportation Success Rate Table", "transportation_success_table.png")
        ]

    def run(self):
        for analyzer, title, filename in self.analyzers:
            df = analyzer.analyze()
            print(f"=== {title} ===")
            print(df)
            analyzer.save_table(df, title, filename)
            print(f"Table saved as {filename}\n")

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", default="table_analyzer.yaml")
