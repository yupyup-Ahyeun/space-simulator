import math
import random
from modules.base_bt_nodes import BTNodeList, Status, Node, Sequence, Fallback, ReactiveSequence, ReactiveFallback, SyncAction, GatherLocalInfo, AssignTask, decision_making_class
...
class IsAllAgents(SyncAction):
    ...
    def _update(self, agent, blackboard):
        ...
        task = agent.tasks_info[assigned_task_id]
        if task.check_arrived_agent_num_completed():
            return Status.SUCCESS
        else:
            return Status.FAILURE
...
class WaitAgents(SyncAction):
    ...
    def _update(self, agent, blackboard):
        ...
        if agent.waiting_now:
            agent.update_waiting_time()
